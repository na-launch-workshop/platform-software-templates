...
Usage:
  python3 register-rhdh-catalog-gitlab.py --gitlab-url https://gitlab.apps.example.com
  python3 register-rhdh-catalog-gitlab.py --gitlab-url https://gitlab.apps.example.com --user devben
  python3 register-rhdh-catalog-gitlab.py --gitlab-url https://gitlab.apps.example.com --dry-run
"""
...
def register_in_rhdh(catalog_url):
    ...
    # Skip if this URL is already registered
    existing = run_psql(f"SELECT 1 FROM locations WHERE target = '{catalog_url}' LIMIT 1;")
    if "(1 row)" in existing:
        return
...
def main():
    ...
    gitlab_host = args.gitlab_url.rstrip("/").replace("https://", "").replace("http://", "")
    ...
    for username in users:
        ...
        for repo in repos:
            name        = repo.get("name")
            # GitLab blob URL format (required by RHDH GitLab catalog integration)
            catalog_url = f"https://{gitlab_host}/{username}/{name}/-/blob/main/catalog-info.yaml"
